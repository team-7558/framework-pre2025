...
public class Traj implements IFollowable {
  ...
  public void generate() {
    if (this.type == TrajType.MOVING) {
      ...
      this.traj = pppath.generateTrajectory(cs, Rotation2d.fromRadians(startrad), config);
  ...
  public void generate(PathPlannerTrajectoryState prevState) {
    if (this.type == TrajType.MOVING) {
      ...
      double rotvel = prevState.fieldSpeeds.omegaRadiansPerSecond;
      ChassisSpeeds cs =
          new ChassisSpeeds(
      ...
      this.traj = pppath.generateTrajectory(cs, prevState.pose.getRotation(), config);
      initState = traj.getInitialState();
    } else {
